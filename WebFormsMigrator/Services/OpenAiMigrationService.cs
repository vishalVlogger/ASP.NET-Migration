...
public sealed class OpenAiMigrationService(HttpClient httpClient, IOptions<OpenAiOptions> options)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        request.Content = JsonContent.Create(new
        {
            ...
            store = false,
            prompt_cache_key = $"reframe-webforms-{batch.Kind}",
        ...
        if (document.RootElement.TryGetProperty("usage", out var usage))
        {
            ...
            {
                InputTokens = usage.TryGetProperty("input_tokens", out var input) ? input.GetInt64() : 0,
                CachedInputTokens = usage.TryGetProperty("input_tokens_details", out var details) && details.TryGetProperty("cached_tokens", out var cached) ? cached.GetInt64() : 0,
                OutputTokens = usage.TryGetProperty("output_tokens", out var output) ? output.GetInt64() : 0
            };
        }
